...
- `Release`
- Ninja
- Every LLVM tool generated by the configured projects
- All non-experimental LLVM targets/backends available in the selected source revision
- `cmake --build ... --target install`

Interactive builds prompt for the LLVM tool selection separately from the LLVM target/backend selection. Press Enter at the tool prompt to keep the default `all` selection. A custom comma-separated tool list keeps `clang` and its resource headers automatically so the result remains a usable managed compiler toolchain, and configured runtimes are still installed.

On Windows, an `all` build or any custom selection containing `llvm-mt` automatically downloads, checksum-verifies, and builds the pinned libxml2 dependency required by upstream LLVM's manifest tool. The resulting install contains both `llvm-mt.exe` and the conventional `mt.exe` compatibility alias.
...
llvm-manager build 22.1.8 --tools clang,lld,llvm-ar,llvm-objdump
llvm-manager build 22.1.8 --tools clang,llvm-mt
...
`--tools all` is the default and uses LLVM's normal `install` target, which installs every tool generated by the configured projects. A custom `--tools` list uses LLVM distribution components to build and install only the requested tool executables plus the compiler support files required by `clang`. `mt` is accepted as shorthand for `llvm-mt`.
...
7. Verifies explicitly selected tool executables. On Windows, a complete tool build additionally requires both `llvm-mt.exe` and `mt.exe`.
